...
import {
  ...
  Shield, Target, Activity, Clock, MapPin, Star
} from "lucide-react";
...
async function saveKey(key, value) {
  try { await window.storage.set(key, JSON.stringify(value)); } catch (e) { console.error("storage error", key, e); }
}
...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      const [c, p, m, t, tk] = await Promise.all([
...
function MatchForm({ ctx, initial, onClose, onSave, onDelete }) {
  ...
  const tabs = [["datos", "Datos"], ["convocatoria", "Convocatoria"], ["eventos", "Cambios/Goles/Tarjetas"], ["tactica", "Táctica"], ["rendimiento", "Rendimiento"], ["aprendizajes", "Aprendizajes"], ["obs", "Observaciones"]];
...
function Configuracion({ ctx }) {
  ...
  const tabs = [["equipo", "Equipo"], ["posiciones", "Posiciones"], ["sistemas", "Sistemas tácticos"], ["ranking", "Fórmula ranking"], ["estados", "Estados jugador"], ["campos", "Campos personalizados"]];
