...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      const [c, p, m, t, tk, rv, se, pl2, st] = await Promise.all([
  ...
  const ctx = { config, updateConfig, players, updatePlayers, matches, updateMatches, trainings, updateTrainings, tasks, updateTasks,
    rivals, updateRivals, season, updateSeason, planning, updatePlanning, standings, updateStandings };
  ...
  return (
    <div className="h-screen w-full flex flex-col overflow-hidden" style={{ background: T.bg, color: T.text, fontFamily: "ui-sans-serif, system-ui, -apple-system, sans-serif" }}>
      ...
      <div className="w-full shrink-0 px-3 py-1.5 text-center text-[11px] sm:text-xs font-bold tracking-wide" style={{ background: T.red, color: "#fff" }}>
        VERSIÓN DE DEMOSTRACIÓN · Podés probar todo libremente — nada se guarda y se reinicia al recargar la página
      </div>
...
function StandingsCard({ standings, onEdit, puntosCasa, puntosFuera }) {
  const s = standings || DEFAULT_STANDINGS;
  const hasData = s.posicion != null || s.puntos != null;
  return (
    <Card className="p-4">
      ...
      {!hasData ? (
        <div className="text-sm" style={{ color: T.muted }}>Todavía no has cargado la posición del equipo. Toca "Cargar datos" para registrarla manualmente, o pega el enlace a la tabla oficial de tu liga.</div>
      ) : (
        <div className="flex flex-col gap-3">
          <div className="flex items-end gap-4 flex-wrap">
            <div>
              <div className="text-3xl font-black" style={{ color: T.navy }}>{s.posicion}º{s.totalEquipos ? <span className="text-base font-bold" style={{ color: T.muted }}> / {s.totalEquipos}</span> : ""}</div>
              <div className="text-[11px] font-bold uppercase" style={{ color: T.muted }}>Posición</div>
            </div>
            <div><div className="text-2xl font-extrabold" style={{ color: T.red }}>{s.puntos ?? "-"}</div><div className="text-[11px] font-bold uppercase" style={{ color: T.muted }}>Puntos</div></div>
            <div><div className="text-2xl font-extrabold" style={{ color: T.navy }}>{s.jugados ?? "-"}</div><div className="text-[11px] font-bold uppercase" style={{ color: T.muted }}>Jugados</div></div>
            <div><div className="text-2xl font-extrabold" style={{ color: T.green }}>{s.ganados ?? "-"}</div><div className="text-[11px] font-bold uppercase" style={{ color: T.muted }}>Ganados</div></div>
            <div><div className="text-2xl font-extrabold" style={{ color: T.yellow }}>{s.empatados ?? "-"}</div><div className="text-[11px] font-bold uppercase" style={{ color: T.muted }}>Empatados</div></div>
            <div><div className="text-2xl font-extrabold" style={{ color: T.redLight }}>{s.perdidos ?? "-"}</div><div className="text-[11px] font-bold uppercase" style={{ color: T.muted }}>Perdidos</div></div>
            <div><div className="text-2xl font-extrabold" style={{ color: T.navy }}>{s.golesFavor ?? "-"}:{s.golesContra ?? "-"}</div><div className="text-[11px] font-bold uppercase" style={{ color: T.muted }}>Goles</div></div>
          </div>
          <div className="grid grid-cols-2 gap-3 pt-3 border-t" style={{ borderColor: T.line }}>
            <div>
              <div className="text-[11px] font-bold uppercase mb-1" style={{ color: T.muted }}>En casa</div>
              <div className="text-sm" style={{ color: T.text }}>{s.casa?.jugados ?? "-"} PJ · {s.casa?.ganados ?? "-"}G {s.casa?.empatados ?? "-"}E {s.casa?.perdidos ?? "-"}P {puntosCasa != null ? `· ${puntosCasa} pts` : ""}</div>
            </div>
            <div>
              <div className="text-[11px] font-bold uppercase mb-1" style={{ color: T.muted }}>Fuera de casa</div>
              <div className="text-sm" style={{ color: T.text }}>{s.fuera?.jugados ?? "-"} PJ · {s.fuera?.ganados ?? "-"}G {s.fuera?.empatados ?? "-"}E {s.fuera?.perdidos ?? "-"}P {puntosFuera != null ? `· ${puntosFuera} pts` : ""}</div>
            </div>
          </div>
...
function StandingsForm({ initial, onClose, onSave }) {
  ...
  return (
    <Modal title="Clasificación en la liga" onClose={onClose} wide>
      <div className="text-sm mb-4" style={{ color: T.muted }}>Puedes cargar los datos manualmente, pegar un enlace a la tabla oficial, o ambos.</div>
      <div className="flex flex-col gap-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          ...
          <Field label="Puntos"><TextInput type="number" value={f.puntos ?? ""} onChange={(e) => setNum("puntos", e.target.value)} /></Field>
          <Field label="Jugados"><TextInput type="number" value={f.jugados ?? ""} onChange={(e) => setNum("jugados", e.target.value)} /></Field>
          <Field label="Ganados"><TextInput type="number" value={f.ganados ?? ""} onChange={(e) => setNum("ganados", e.target.value)} /></Field>
          <Field label="Empatados"><TextInput type="number" value={f.empatados ?? ""} onChange={(e) => setNum("empatados", e.target.value)} /></Field>
          <Field label="Perdidos"><TextInput type="number" value={f.perdidos ?? ""} onChange={(e) => setNum("perdidos", e.target.value)} /></Field>
          <Field label="Goles a favor"><TextInput type="number" value={f.golesFavor ?? ""} onChange={(e) => setNum("golesFavor", e.target.value)} /></Field>
          <Field label="Goles en contra"><TextInput type="number" value={f.golesContra ?? ""} onChange={(e) => setNum("golesContra", e.target.value)} /></Field>
        </div>
        <div>
          <Eyebrow>En casa</Eyebrow>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-2">
            <Field label="Jugados"><TextInput type="number" value={f.casa?.jugados ?? ""} onChange={(e) => setHomeAway("casa", "jugados", e.target.value)} /></Field>
            <Field label="Ganados"><TextInput type="number" value={f.casa?.ganados ?? ""} onChange={(e) => setHomeAway("casa", "ganados", e.target.value)} /></Field>
            <Field label="Empatados"><TextInput type="number" value={f.casa?.empatados ?? ""} onChange={(e) => setHomeAway("casa", "empatados", e.target.value)} /></Field>
            <Field label="Perdidos"><TextInput type="number" value={f.casa?.perdidos ?? ""} onChange={(e) => setHomeAway("casa", "perdidos", e.target.value)} /></Field>
          </div>
        </div>
        <div>
          <Eyebrow>Fuera de casa</Eyebrow>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-2">
            <Field label="Jugados"><TextInput type="number" value={f.fuera?.jugados ?? ""} onChange={(e) => setHomeAway("fuera", "jugados", e.target.value)} /></Field>
            <Field label="Ganados"><TextInput type="number" value={f.fuera?.ganados ?? ""} onChange={(e) => setHomeAway("fuera", "ganados", e.target.value)} /></Field>
            <Field label="Empatados"><TextInput type="number" value={f.fuera?.empatados ?? ""} onChange={(e) => setHomeAway("fuera", "empatados", e.target.value)} /></Field>
            <Field label="Perdidos"><TextInput type="number" value={f.fuera?.perdidos ?? ""} onChange={(e) => setHomeAway("fuera", "perdidos", e.target.value)} /></Field>
          </div>
        </div>
      </div>
...
function Dashboard({ ctx, goto }) {
  ...
  // Usa la clasificación cargada manualmente si existe; si no, calcula desde los partidos registrados en la app
  const s = standings || {};
  const hasManualRecord = s.ganados != null || s.empatados != null || s.perdidos != null;
  const v = hasManualRecord ? (s.ganados ?? 0) : teamStats.v;
  const e = hasManualRecord ? (s.empatados ?? 0) : teamStats.e;
  const d = hasManualRecord ? (s.perdidos ?? 0) : teamStats.d;
  const jugadosRecord = v + e + d;
  const pctVictorias = jugadosRecord ? Math.round((v / jugadosRecord) * 100) : 0;

  const hasManualGoals = s.golesFavor != null || s.golesContra != null;
  const gf = hasManualGoals ? (s.golesFavor ?? 0) : teamStats.gf;
  const gc = hasManualGoals ? (s.golesContra ?? 0) : teamStats.gc;

  const puntosCasa = s.casa ? (s.casa.ganados ?? 0) * 3 + (s.casa.empatados ?? 0) : null;
  const puntosFuera = s.fuera ? (s.fuera.ganados ?? 0) * 3 + (s.fuera.empatados ?? 0) : null;
  ...
  return (
    <div className="flex flex-col gap-6 max-w-6xl">
      ...
      <StandingsCard standings={standings} onEdit={() => setEditingStandings(true)} puntosCasa={puntosCasa} puntosFuera={puntosFuera} />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <Card className="p-4">
          <Eyebrow>Récord{hasManualRecord ? "" : " (según partidos de la app)"}</Eyebrow>
        ...
        <Card className="p-4">
          <Eyebrow>Goles{hasManualGoals ? "" : " (según partidos de la app)"}</Eyebrow>
...
function MatchDetailView({ match: m, players, config, onClose, onEdit, onDelete }) {
  ...
  const [gf, gc] = (m.resultadoFinal || "0-0").split("-").map(Number);
  const resColor = gf > gc ? T.green : gf === gc ? T.yellow : T.redLight;
...
function PitchEditor({ system, players, lineup, onChange, readOnly = false }) {
  ...
  return (
    <div>
      ...
      {!readOnly && <div className="text-xs mt-2" style={{ color: T.muted }}>Toca un jugador para asignarlo. Mantén pulsado y arrastra para moverlo sobre el campo.</div>}
...
function printTask(t, config) {
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    body { font-family: -apple-system, Segoe UI, Roboto, Arial, sans-serif; color: #1B2733; padding: 16px; }
    .toolbar { position: sticky; top: 0; display: flex; justify-content: center; gap: 10px; padding: 10px 0 16px; z-index: 50; }
    .toolbar button { font-family: inherit; font-size: 14px; font-weight: 800; border: none; border-radius: 12px; padding: 12px 22px; cursor: pointer; box-shadow: 0 4px 14px rgba(0,0,0,0.18); }
    .btnPrint { background: #E63946; color: #fff; }
    .btnClose { background: #fff; color: #0B1F3A; border: 1.5px solid #CBD5E1 !important; }
    .sheet { max-width: 210mm; margin: 0 auto; background: #fff; border-radius: 16px; padding: 22px 26px 28px; box-shadow: 0 6px 24px rgba(11,31,58,0.15); }
    .header { display: flex; align-items: center; gap: 14px; border-bottom: 3px solid #0B1F3A; padding-bottom: 14px; margin-bottom: 16px; }
    .header img { width: 54px; height: 54px; object-fit: contain; border-radius: 12px; }
    .headerInfo { flex: 1; text-align: left; }
    .teamName { font-size: 17px; font-weight: 800; color: #0B1F3A; }
    .teamMeta { font-size: 11px; color: #64748B; margin-top: 2px; }
    .title { text-align: center; margin: 4px 0 18px; }
    .title h1 { font-size: 22px; font-weight: 900; color: #E63946; margin: 0; letter-spacing: 0.5px; text-transform: uppercase; }
    .title .sub { font-size: 11.5px; color: #64748B; margin-top: 3px; font-weight: 600; }
    .taskName { font-size: 18px; font-weight: 800; color: #0B1F3A; text-align: center; }
    .taskCode { font-size: 11.5px; color: #64748B; text-align: center; margin-bottom: 16px; }
    .grid { display: grid; grid-template-columns: 1fr 1fr; gap: 6px 22px; background: #F5F7FA; border-radius: 12px; padding: 12px 16px; }
    .spec { display: flex; justify-content: space-between; font-size: 11.5px; padding: 4px 0; border-bottom: 1px dotted #D8DEE7; }
    .specLabel { color: #64748B; font-weight: 700; }
    .specValue { color: #0B1F3A; font-weight: 800; text-align: right; }
    .section { margin-top: 18px; }
    .sectionTitle { font-size: 11.5px; font-weight: 800; text-transform: uppercase; color: #fff; padding: 6px 14px; border-radius: 999px; display: inline-block; margin-bottom: 10px; letter-spacing: 0.3px; }
    .objectives { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; }
    .block { background: #FAFBFC; border: 1.5px solid #E2E8F0; border-radius: 12px; padding: 10px 14px; margin-bottom: 10px; }
    .blockLabel { font-size: 10px; font-weight: 800; text-transform: uppercase; margin-bottom: 3px; }
    .blockText { font-size: 12.5px; line-height: 1.45; color: #1B2733; }
    .imgWrap { text-align: center; margin: 6px 0; }
    .imgWrap img { max-width: 100%; max-height: 280px; border-radius: 14px; border: 1.5px solid #E2E8F0; }
    .footer { margin-top: 22px; text-align: center; font-size: 9.5px; color: #94A3B8; border-top: 1px solid #E2E8F0; padding-top: 10px; }
    @media print {
    ...
    <div class="sheet">
      <div class="header">
        ...
        <div class="headerInfo">
          ...
          <div class="teamMeta">${esc(config.category || "")}${config.category && config.season ? " · " : ""}${esc(config.season || "")}${(config.category || config.season) ? " · " : ""}Metodología PZ · Puntualización Zonal</div>
        </div>
      </div>
      ...
      ${section("Representación gráfica", "#2A9D5C", t.imagen ? `<div class="imgWrap"><img src="${t.imagen}" /></div>` : "")}
