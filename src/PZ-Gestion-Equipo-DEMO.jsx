      ...
      {!hasData ? (
        ...
        <div className="flex flex-col gap-3">
          <div className="flex gap-2 flex-wrap">
            ...
            <Cell value={s.puntos ?? "-"} label="Puntos" color={T.red} />
            <Cell value={s.jugados ?? "-"} label="Jugados" color={T.navy} />
            <Cell value={s.ganados ?? "-"} label="Ganados" color={T.green} />
            <Cell value={s.empatados ?? "-"} label="Empatados" color={T.yellow} />
            <Cell value={s.perdidos ?? "-"} label="Perdidos" color={T.redLight} />
            <Cell value={`${s.golesFavor ?? "-"}:${s.golesContra ?? "-"}`} label="Goles" color={T.navy} />
          </div>
          <div className="grid grid-cols-2 gap-3 pt-3 border-t" style={{ borderColor: T.line }}>
            <div className="rounded-xl p-2.5" style={{ background: "#F5F7FA", border: `1px solid ${T.line}` }}>
              ...
              <div className="flex justify-center gap-1.5 flex-wrap">
                <Cell value={s.casa?.jugados ?? "-"} label="PJ" color={T.navy} />
                <Cell value={s.casa?.ganados ?? "-"} label="G" color={T.green} />
                <Cell value={s.casa?.empatados ?? "-"} label="E" color={T.yellow} />
                <Cell value={s.casa?.perdidos ?? "-"} label="P" color={T.redLight} />
                {puntosCasa != null && <Cell value={puntosCasa} label="Pts" color={T.red} />}
              </div>
            </div>
            <div className="rounded-xl p-2.5" style={{ background: "#F5F7FA", border: `1px solid ${T.line}` }}>
              <div className="text-[11px] font-bold uppercase mb-1.5 text-center" style={{ color: T.muted }}>Fuera de casa</div>
              <div className="flex justify-center gap-1.5 flex-wrap">
                <Cell value={s.fuera?.jugados ?? "-"} label="PJ" color={T.navy} />
                <Cell value={s.fuera?.ganados ?? "-"} label="G" color={T.green} />
                <Cell value={s.fuera?.empatados ?? "-"} label="E" color={T.yellow} />
                <Cell value={s.fuera?.perdidos ?? "-"} label="P" color={T.redLight} />
                {puntosFuera != null && <Cell value={puntosFuera} label="Pts" color={T.red} />}
              </div>
            </div>
          </div>
...
function printMatch(m, config, players) {
  ...
  const resColor = gf > gc ? "#2A9D5C" : gf === gc ? "#F4B400" : "#FF6B6B";
  ...
  <body>
    ...
    <div class="sheet">
      ...
      <div class="twoCol">
        <div>
          <div class="section">
            <div class="sectionTitle" style="background:#0B1F3A;">Convocatoria (${convocados.length})</div>
            <div>${convocados.map((c) => `<span class="pill">${esc(playerName(c.jugadorId))} · ${esc(c.estado)}${m.capitanId === c.jugadorId ? " (C)" : ""}</span>`).join("")}</div>
          </div>
    ...
    <Modal title={initial ? "Editar partido" : "Nuevo partido"} onClose={onClose} wide>
      ...
      {tab === "convocatoria" && (
        <div className="flex flex-col gap-1.5">
          <div className="text-xs mb-1" style={{ color: T.muted }}>{f.convocatoria.filter((c) => c.estado === "Convocado").length} jugadores convocados. Tocá la ⭐ para marcar al capitán.</div>
          {players.map((p) => {
...
function printTraining(t, config, players) {
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .taskRowImg { width: 42px; height: 42px; object-fit: cover; border-radius: 6px; border: 1px solid #E2E8F0; flex-shrink: 0; }
