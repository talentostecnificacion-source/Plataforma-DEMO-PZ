...
const DEFAULT_SEASON_PLANNING = {
  pretemporada: { fechaInicio: "", fechaFin: "", dias: [], amistosos: [] },
  mensual: [],
  macrociclo: [],
  partidoNoProgramado: [],
  torneoRapido: [],
...
function printTask(t, config) {
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .twoCol { display: grid; grid-template-columns: 1fr 150px; gap: 14px; align-items: start; }
    ...
    .imgWrap img { width: 100%; max-height: 140px; object-fit: cover; border-radius: 10px; border: 1.5px solid #E2E8F0; }
  ...
  <body>
    ...
    <div class="sheet" id="sheet">
      ...
      <div class="twoCol">
        ...
        <div>
          ${t.imagen ? `<div class="sideLabel">Representación gráfica</div><div class="imgWrap"><img src="${t.imagen}" /></div>` : ""}
          ${t.observaciones ? `<div class="sideLabel">Notas adicionales</div><div class="block" style="margin-bottom:0;">${esc(t.observaciones)}</div>` : ""}
        </div>
      </div>
...
function PlanItemModalForm({ title, fields, initial, onClose, onSave, onDelete }) {
  const [f, setF] = useState(initial);
  const set = (k, v) => setF((p) => ({ ...p, [k]: v }));
  return (
        ...
        {fields.map((fld) => (
          <Field key={fld.key} label={fld.label} className={fld.wide ? "sm:col-span-2" : ""}>
            {fld.type === "textarea" ? <TextArea value={f[fld.key] || ""} onChange={(e) => set(fld.key, e.target.value)} />
              ...
              : <TextInput value={f[fld.key] || ""} onChange={(e) => set(fld.key, e.target.value)} />}
          </Field>
        ))}
      </div>
      <div className="flex justify-between gap-2 mt-5 sticky bottom-0 bg-white -mx-5 px-5 py-3 border-t" style={{ borderColor: T.line }}>
...
function SimplePlanList({ title, items, blank, fields, titleField, subtitleField, onAdd, onSave, onRemove }) {
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {showNew && <PlanItemModalForm title={title} fields={fields} initial={blank()} onClose={() => setShowNew(false)} onSave={(it) => { onAdd(it); setShowNew(false); }} />}
      {editing && <PlanItemModalForm title={title} fields={fields} initial={editing} onClose={() => setEditing(null)} onSave={(it) => { onSave(it); setEditing(null); }} onDelete={() => { onRemove(editing.id); setEditing(null); }} />}
    </div>
  );
}

function TemporadaModule({ ctx }) {
  const { season, updateSeason, matches, trainings, seasonPlanning, updateSeasonPlanning } = ctx;
        ...
        <div className="flex flex-col gap-4">
          ...
          {planTab === "pretemporada" && (
            <div className="flex flex-col gap-4">
              ...
              <div>
                <div className="flex items-center justify-between mb-2"><Eyebrow>Partidos amistosos</Eyebrow><Btn variant="ghost" icon={Plus} onClick={() => setAmistosoModal({ id: uid(), fecha: todayISO(), rival: "", lugar: "", objetivo: "" })}>Añadir amistoso</Btn></div>
                <div className="flex flex-col gap-2">
              ...
              {amistosoModal && (
                <PlanItemModalForm title="Partido amistoso"
                  fields={[{ key: "fecha", label: "Fecha", type: "date" }, { key: "rival", label: "Rival" }, { key: "lugar", label: "Lugar" }, { key: "objetivo", label: "Objetivo del amistoso", type: "textarea", wide: true }]}
          ...
          {planTab === "imprevisto" && (
            <SimplePlanList title="Partido no programado"
              ...
              blank={() => ({ id: uid(), fecha: todayISO(), rival: "", objetivo: "", ajustesTacticos: "" })}
              fields={[{ key: "fecha", label: "Fecha", type: "date" }, { key: "rival", label: "Rival" }, { key: "objetivo", label: "Objetivo del partido", type: "textarea", wide: true }, { key: "ajustesTacticos", label: "Ajustes tácticos rápidos", type: "textarea", wide: true }]}
          ...
          {planTab === "torneo" && (
            <SimplePlanList title="Torneo rápido"
              ...
              blank={() => ({ id: uid(), nombreTorneo: "", fechaInicio: "", fechaFin: "", partidos: "", rotacion: "" })}
              fields={[{ key: "nombreTorneo", label: "Nombre del torneo" }, { key: "fechaInicio", label: "Fecha de inicio", type: "date" }, { key: "fechaFin", label: "Fecha de fin", type: "date" }, { key: "partidos", label: "Partidos (fechas y rivales)", type: "textarea", wide: true }, { key: "rotacion", label: "Plan de rotación / manejo de la fatiga", type: "textarea", wide: true }]}
